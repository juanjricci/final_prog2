  ...
  return (
    <div>
      <h2 id="venta-heading" data-cy="VentaHeading">
        <Translate contentKey="carroComprasApp.venta.home.title">Ventas</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="carroComprasApp.venta.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="carroComprasApp.venta.home.createLabel">Create new Venta</Translate>
          </Link>
        </div>
      </h2>
          ...
          {ventaList && ventaList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="carroComprasApp.venta.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('fechaVenta')}>
                    <Translate contentKey="carroComprasApp.venta.fechaVenta">Fecha Venta</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('precioTotal')}>
                    <Translate contentKey="carroComprasApp.venta.precioTotal">Precio Total</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="carroComprasApp.venta.idCarroCompra">Id Carro Compra</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {ventaList.map((venta, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>
                      {venta.idCarroCompra ? <Link to={`carro-compra/${venta.idCarroCompra.id}`}>{venta.idCarroCompra.id}</Link> : ''}
                    </td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="carroComprasApp.venta.home.notFound">No Ventas found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
